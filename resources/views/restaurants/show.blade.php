...
{{__('messages.create_category')}}
    ...
    <div class="flex flex-wrap justify-center">
        ...
        <div class="p-5">
            <a href="{{route('category.show',[$restaurant,$category])}}">
                <h5 class="h-16 text-base font-bold tracking-tight text-gray-900 dark:text-white">{{$category->name}}</h5>
            </a>
   ...
   <button type="button" @click="deleteItem(`{{route('category.destroy',[$restaurant,$category])}}`)" class="inline-flex items-center px-3 py-2 text-sm font-medium text-center text-white bg-red-500 rounded-lg hover:bg-red-800 focus:ring-4 focus:outline-none focus:ring-red-300 dark:bg-red-600 dark:hover:bg-red-700 dark:focus:ring-red-800" x-data="{
             deleteItem(url) {
